...
RAM_HOLDER_AMOUNT = 512000000 # reserve 512Mb of Memory
...
class Validator():
    ...
    def __init__(self, app):
        ...
        if validation_printing_enabled:
            print("[Validation {}] sub routine initialized, reserving ram".format(dt.datetime.now()))

        # ramholder = bytearray(RAM_HOLDER_AMOUNT)
        ...
        while(_isUploaded == False or _uploadTrials < 5):
            ...
            if(res == ""):
                try:
                    ...
                    trials_ = 0
                    bucket_to_upload = "exorde-spotcheck-1"
                    while True:
                        time.sleep(2)
                        try:
            ...
            try:
                if general_printing_enabled:
                    if ( now_ts -self._rewardsInfoLastTimestamp ) > delay_between_rewardsInfo or self._rewardsInfoLastTimestamp == 0: 
                        ...
                        time.sleep(0.5) 
                        rep_amount = round(self.app.cm.instantiateContract("Reputation").functions.balanceOf(main_addr).call()/(10**18),2)
                        print("[CURRENT REWARDS & REP] Main Address {}, REP = {} and EXDT Rewards = {} , Time of display = {}".format(str(main_addr), rep_amount, exdt_rewards, dt.datetime.now()))
                        self._rewardsInfoLastTimestamp = now_ts
            except:
                time.sleep(2)
                pass
